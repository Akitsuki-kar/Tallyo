...
本文档说明如何获取应用密码、填写地址，以及**不同运行环境的正确填法**：

| 运行环境 | 地址填什么 | 说明 |
|---|---|---|
| 开发（`npm run dev`） | `/dav/水电动账` | Vite 代理同源转发，规避浏览器 CORS |
| Web / PWA 生产（已部署站点） | `/dav/水电动账` | 需站点同域名下配 Nginx 反代（见第 4 节） |
| **Windows / macOS 原生客户端**（Tauri 壳） | **完整 HTTPS 地址** `https://dav.jianguoyun.com/dav/水电动账` | 走原生 HTTP 通道，无 CORS，**无需反代**（见第 3 节） |
...
### Web / PWA（浏览器环境）：填同源相对路径

浏览器（含 WebView）直连跨域地址会被 CORS 拦截，因此 Web 端必须让请求看起来是**同源**的：
...
> ⚠️ **Web 端不要填跨域绝对地址**（如 `https://dav.jianguoyun.com/dav/...`）。
> 本应用的 CSP 设置为 `connect-src 'self'`，跨域请求会被浏览器安全策略直接拦截，
> 即使反代配置完全正确也无法连通。保持「同源 + 相对路径」即可同时满足 CSP 与 CORS。

### Windows / macOS 原生客户端（Tauri 壳）：直接填完整 HTTPS 地址

桌面包**没有**浏览器 CORS、也**没有**服务器可反代，但它内置了**原生 HTTP 通道**
（tauri-plugin-http，由系统底层直接发起请求），完全不受 CORS/CSP 限制，所以直接填：

```
https://dav.jianguoyun.com/dav/水电动账
```

例如：
- 地址：`https://dav.jianguoyun.com/dav/水电动账`
- 用户名：`你的坚果云邮箱`
- 应用密码：第 1 节生成的应用密码

> 若在原生客户端误填相对路径（`/dav/...`），App 会提示「请填写完整 HTTPS 地址」，
> 因为原生壳没有本地服务器可解析相对路径。

实际数据文件会存放在：`<地址>/水电动账/data.json`，目录会在首次同步时自动创建。

---

## 3. 为什么直连坚果云会 CORS 失败，以及两种解决路径
...
1. **Web / PWA**：同源反代 —— 请求对浏览器而言是**同源**（`你的域名/dav/...`），
   既不需要 CORS 头，也不受 CSP `connect-src 'self'` 限制。跨域发生在「你的服务器 → 坚果云」之间，
   对浏览器完全透明。
2. **原生客户端（Tauri 桌面/移动壳）**：原生 HTTP 通道 —— App 检测到 Tauri 环境时，
   WebDAV 请求自动改走 `tauri-plugin-http`（Rust reqwest），由系统网络栈发出，**不经浏览器**，
   天然没有 CORS/CSP 概念，可直接访问任意 HTTPS 的 WebDAV 端点（包括坚果云）。

> 旧版本桌面包会报「网络请求失败（可能是 CORS 拦截或断网）」——
> 那是 WebView 内浏览器 fetch 被 CORS 拦截所致。升级到含原生 HTTP 通道的版本后，
> 原生客户端填完整 HTTPS 地址即可直接连通，无需任何代理。

---

## 4. Web / PWA 生产部署：同源 Nginx 反向代理示例

> 本节仅适用于 **Web / PWA 站点**；原生客户端**无需**任何代理配置，可跳过本节。
...
- 不要手动高频点击「立即同步」。
- 多设备不要在同一秒同时触发大量写入。
- 若遇到持续同步失败，先检查应用密码是否失效、目录是否存在，再参考本指南核对配置。
